  
原始数据集为LabelImage标注的xml格式，这里我们对其进行预处理，得到常用的coco格式。

其中，`PaddleDetection`提供了`x2coco.py`用于将`voc`, `widerface`, `labelme` 或者 `cityscape`转换为COCO格式。在转换之前我们需要提供训练集和验证集标签的id文件
...
        --voc_out_name /home/aistudio/Car2024/val.json
```
